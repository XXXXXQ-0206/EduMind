...
async def teaching_video_websocket(websocket: WebSocket):
    ...
    await manager.send_message({"type": "ready", "videoId": video_id}, video_id)
    ...
    try:
        ...
        if not topic:
            await manager.send_message({"type": "error", "error": "Topic not found"}, video_id)
            await websocket.close(code=1000, reason="Topic not found")
            return

        async def _emit_saved_state() -> tuple[bool, bool]:
            """回放已保存的阶段结果，返回 (has_final_media_or_error, has_script)."""
            ...
            if isinstance(saved_phase, str) and saved_phase.strip():
                await manager.send_message({"type": "phase", "value": saved_phase.strip()}, video_id)
            if isinstance(saved_script, str) and saved_script.strip():
                await manager.send_message({"type": "script", "script": saved_script}, video_id)
            if isinstance(saved_audio, str) and saved_audio.strip():
                await manager.send_message({"type": "local_audio", "audioPath": saved_audio}, video_id)
            if isinstance(saved_video, str) and saved_video.strip():
                await manager.send_message({"type": "video", "videoUrl": saved_video}, video_id)
            if isinstance(saved_local, str) and saved_local.strip():
                await manager.send_message({"type": "local_video", "localPath": saved_local}, video_id)
            if isinstance(saved_error, str) and saved_error.strip():
                await manager.send_message({"type": "video_error", "error": saved_error}, video_id)

            has_final = bool((saved_video and str(saved_video).strip()) or (saved_local and str(saved_local).strip()) or (saved_audio and str(saved_audio).strip()) or (saved_error and str(saved_error).strip()))
        ...
        if run_state in {"running", "done", "failed"}:
            has_final, has_script = await _emit_saved_state()
            if run_state in {"done", "failed"} and (has_final or has_script):
                await manager.send_message({"type": "done"}, video_id)
                return
            if run_state == "running":
                # 任务正在另一个连接中执行，不重复启动，等待其收敛并回放结果
                for _ in range(240):
                    await asyncio.sleep(2.0)
                    next_state = str(await json_storage.get(f"video:{video_id}:run_state") or "").strip().lower()
                    has_final, has_script = await _emit_saved_state()
                    if next_state in {"done", "failed"} and (has_final or has_script):
                        await manager.send_message({"type": "done"}, video_id)
                        return
                await manager.send_message({"type": "error", "error": "video_generation_timeout"}, video_id)
                return

        await json_storage.set(f"video:{video_id}:run_state", "running")

        await manager.send_message({"type": "phase", "value": "script"}, video_id)
        ...
        input_data = TeachingVideoInput(topic=topic, materials_context=materials_context or None)
        ...
        if result.success and result.script:
            ...
            await manager.send_message({"type": "script", "script": result.script}, video_id)

            provider_name = get_configured_provider()
            profile = _video_profile(agent, topic, result.script)

            if provider_name != "gateway":
                await manager.send_message({"type": "phase", "value": "audio"}, video_id)
                ...
                if audio_path:
                    ...
                    await manager.send_message({"type": "local_audio", "audioPath": audio_path}, video_id)

            if jiemeng_video_configured():
                await manager.send_message({"type": "phase", "value": "video"}, video_id)
                ...
                try:
                        ...
                        try:
                            ...
                            if provider_choice == "visual" and _is_visual_text_risk_error(provider_exc):
                                try:
                                    ...
                                    logger.warning(
                                        "teaching_video visual risk retry topic=%s reason=%s",
                                        topic,
                                        _format_video_error(provider_exc),
                                    )
                                    task = await submit_text_to_video(
                                        safe_prompt,
                                        duration_sec=10,
                            ...
                            if idx < len(provider_candidates) - 1:
                                logger.warning(
                                    "teaching_video provider switch topic=%s from=%s to=%s reason=%s",
                                    topic,
                                    provider_choice,
                                    provider_candidates[idx + 1],
                                    _format_video_error(provider_exc),
                                )
                                continue
                            raise provider_exc
                    ...
                    if status == "done" and video_url:
                        ...
                        meta["updated_at"] = __import__("datetime").datetime.now().isoformat()
                        await json_storage.set(f"video:{video_id}", meta)
                        await manager.send_message({"type": "video", "videoUrl": video_url}, video_id)

                        local_path = None
                        if task.provider == "gateway":
                            local_path = await cache_remote_teaching_video(video_id, video_url)
                        else:
                            local_path = await add_audio_to_teaching_video(video_id, video_url, result.script)
                            if local_path:
                        ...
                        if local_path:
                            ...
                            await manager.send_message({"type": "local_video", "localPath": local_path}, video_id)
                    else:
                    ...
                    await manager.send_message({"type": "video_error", "error": error_message}, video_id)
                    if config.jimeng_video_allow_fallback:
                        await manager.send_message({"type": "phase", "value": "video_pending"}, video_id)
                        ...
                        try:
                            ...
                            if fallback_path:
                                ...
                                await manager.send_message({"type": "local_video", "localPath": fallback_path}, video_id)
                            else:
                                logger.warning("teaching_video fallback slideshow unavailable video_id=%s", video_id)
            ...
            await manager.send_message({"type": "error", "error": result.error or "script_failed"}, video_id)

    except WebSocketDisconnect:
        manager.disconnect(websocket, video_id)
    except Exception as e:
        import traceback
        print(f"[ERROR] Teaching video websocket: {type(e).__name__}: {e!r}")
        traceback.print_exc()
        try:
            await json_storage.set(f"video:{video_id}:run_state", "failed")
            await json_storage.set(f"video:{video_id}:phase", "error")
            ...
            pass
        try:
            await manager.send_message({"type": "error", "error": str(e)}, video_id)
        except Exception:
            pass
    finally:
        manager.disconnect(websocket, video_id)
...
async def get_teaching_video_handler(video_id: str, user: AuthUser = Depends(require_auth)):
    ...
    try:
        ...
        local_audio_url = ""
        if local_path:
            full = config.storage_dir / local_path
            if full.exists():
                local_video_url = f"/storage/{local_path}"
        if audio_path:
            full_audio = config.storage_dir / audio_path
            if full_audio.exists():
                local_audio_url = f"/storage/{audio_path}"
...
async def stream_teaching_video_handler(
    ...
    local_path = await json_storage.get(f"video:{video_id}:local_path")
    if local_path:
        full = Path(config.storage_dir) / local_path
        if full.exists():
