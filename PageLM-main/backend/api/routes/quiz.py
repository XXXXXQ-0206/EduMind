...
from utils.storage import get_quiz_attempts, json_storage, list_files_for_user, list_quizzes, owner_payload, record_belongs_to_user, set_quiz_attempts
...
async def create_quiz(request: QuizRequest, user: AuthUser = Depends(require_auth)):
    ...
    try:
        ...
        ensure_quiz_generation_task(quiz_id)
...
async def _safe_send_quiz(quiz_id: str, msg: Dict[str, Any]) -> None:
    try:
        await manager.send_message(msg, quiz_id)
    except Exception as e:
...
def ensure_quiz_generation_task(quiz_id: str) -> None:
...
async def upsert_quiz_attempt_answer_handler(quiz_id: str, request: QuizAttemptAnswerRequest, user: AuthUser = Depends(require_auth)):
    ...
    try:
        ...
        for idx, item in enumerate(attempts):
            if item.get("questionId") == payload.get("questionId"):
        ...
        await set_quiz_attempts(quiz_id, attempts)
        meta = await json_storage.get(f"quiz:{quiz_id}") or {}
        if meta:
            meta["updated_at"] = datetime.now().isoformat()
            await json_storage.set(f"quiz:{quiz_id}", meta)
