...
class SlideItem:
    """单页幻灯片数据"""
    def __init__(self, title: str, bullets: List[str], image_path: Optional[str] = None):
        self.title = title
        ...
        try:
            ...
            try:
                ...
                for i, slide in enumerate(slides_data):
                    ...
                    img_path = None
                    # 仅使用即梦真实图。超时/失败则留空，不再生成占位图。
                    try:
                        img_path = await asyncio.wait_for(
                            generate_image(prompt, save_dir=save_dir), timeout=35
                        )
                    except asyncio.TimeoutError:
                        img_path = None
                        logger.warning("即梦配图超时 slide_id=%s page=%s", slide_id, i + 1)
                    if img_path:
                        generated_count += 1
                    else:
                        skipped_count += 1
                        continue
                    slide.image_path = f"/storage/slides/{slide_id}/{img_path.name}"
                if skipped_count:
                    logger.warning(
