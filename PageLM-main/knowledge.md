...
| 文件 | 作用 |
|------|------|
| `mcpManager.js` | MCP Client 单例：使用 `StdioClientTransport` 连接 `bilibili-mcp-js-main/dist/index.js`，初始化工具并执行视频检索 |
...
| 页面 | 路由 | 说明 |
|------|------|------|
| `LinkEduHome.vue` | `/` | 首页：双入口按钮（教师/学生）先 setRole 再 push 对应对话页 |
...
- **实现**：`GET /files?role=teacher` 列表；`POST /files` 多文件上传，存 `storage/uploads/`，元数据写 JSON；`DELETE /files/{id}`。前端可勾选“教学文件夹”，对应 key `pagelm-learning-folder-teacher`，供教案/测验/对话等“关联资料”使用。
...
- **路由**：`/chat`。
- **实现**：与教师端同一套后端，`role=student`；`scope=student` 的会话独立存储；前端 `chatJSON({ role: 'student', ... })`；学习资料文件夹 key 为 `pagelm-learning-folder`；欢迎区标题“学生控制台”。
- **技术**：与 3.1 相同，仅 role/scope 与前端文案不同。

### 4.2 文件库（学生）

- **路由**：`/file-library`。
- **实现**：`GET/POST/DELETE /files?role=student`；学生端“学习文件夹”key 为 `pagelm-learning-folder`，供对话、笔记、测验等挂载。
