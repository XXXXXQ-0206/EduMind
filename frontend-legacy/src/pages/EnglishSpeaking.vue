<template>
  <div class="speaking-page speaking-body relative min-h-screen overflow-hidden px-4 md:pl-60 md:pr-6 lg:pl-64 lg:pr-12">
    ...
    <div class="relative z-10 mx-auto flex w-full max-w-6xl flex-col gap-10 py-16">
      ...
      <div v-if="generateError" class="speaking-panel border border-rose-200 bg-rose-50 text-rose-700 text-sm">
        {{ generateError }}
      </div>
      ...
      <template v-else>
        ...
        <section class="clay-card speaking-fade">
          <div v-if="currentItem" class="flex flex-col gap-6">
            ...
            <div class="flex flex-wrap items-center gap-3">
              <button type="button" class="speaking-pill speaking-pill--primary" @click="playAudio(currentItem.audioUrl, currentItem.text)">
                播放示范
              </button>
              <button
                v-if="!isRecording"
...
const selectedVoice = ref("american");
...
const playAudio = (url: string, fallbackText?: string) => {
  if (!url && fallbackText && "speechSynthesis" in window) {
    try {
      window.speechSynthesis.cancel();
      const utter = new SpeechSynthesisUtterance(fallbackText);
      utter.lang = selectedVoice.value === "british" ? "en-GB" : "en-US";
      utter.rate = 0.95;
      window.speechSynthesis.speak(utter);
      return;
    } catch {
      generateError.value = "示范音频暂不可用，请稍后重试";
      return;
    }
  }
  if (!url) {
    generateError.value = "示范音频生成中，请稍后重试";
    return;
  }
...
const getUserMediaCompat = async (): Promise<MediaStream> => {
...
const startRecording = async () => {
  ...
  try {
    ...
    const insecureContext = !window.isSecureContext && !/^https?:\/\/(localhost|127\.0\.0\.1)(:\d+)?/i.test(window.location.origin);
    if (insecureContext) {
      recordingError.value = "当前环境不支持麦克风直录，已切换为上传音频评测。";
      triggerAudioFileSelect();
      return;
    }
    if (err instanceof DOMException) {
      if (err.name === "NotAllowedError") {
        recordingError.value = "麦克风权限被拒绝，请在浏览器地址栏开启麦克风权限后重试。";
        return;
      }
      if (err.name === "NotFoundError") {
        recordingError.value = "未检测到可用麦克风设备，请连接麦克风后重试。";
        return;
      }
      if (err.name === "NotReadableError") {
        recordingError.value = "麦克风正被其他应用占用，请关闭占用程序后重试。";
        return;
      }
      if (err.name === "SecurityError" || (!window.isSecureContext && err.name === "NotAllowedError")) {
        recordingError.value = "当前页面安全级别不足，请使用 HTTPS 地址或系统浏览器打开。";
        return;
      }
    }
    if (err instanceof Error && err.message === "no_get_user_media") {
      recordingError.value = "当前浏览器不支持直接录音，已切换为上传音频评测。";
      triggerAudioFileSelect();
      return;
    }
...
const evaluateFromFile = async (file: File) => {
