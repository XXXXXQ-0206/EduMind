...
if sys.platform == "win32":
    _workspace_render_tmp = os.path.abspath(os.path.join(os.path.dirname(__file__), "tmp"))
    os.makedirs(_workspace_render_tmp, exist_ok=True)
    os.environ["TEMP"] = _workspace_render_tmp
    os.environ["TMP"] = _workspace_render_tmp
    os.environ["TMPDIR"] = _workspace_render_tmp
    tempfile.tempdir = _workspace_render_tmp
...
def rasterize(
    ...
    tmp_root = join(os.path.dirname(__file__), "tmp")
    run_root = join(tmp_root, f"render_{os.getpid()}_{int(time.time() * 1000)}")
    user_profile = join(run_root, "profile")
    convert_tmp_dir = join(run_root, "convert")
