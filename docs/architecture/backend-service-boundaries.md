...
`identity` 服务持有账户和会话，并将账户库写入 PostgreSQL。旧 `storage/edumind.sqlite3` 仅作为启动迁移来源；其他边界服务在 Compose 中运行 `AUTH_VALIDATION_MODE=remote`，通过 `IDENTITY_URL/auth/internal/resolve` 解析 Bearer token，不直接读取账户库。
...
| 向量存储 | `VECTOR_STORE_PROVIDER` | `pgvector`，使用 PostgreSQL pgvector | legacy JSON vector import |
...
- `backend/utils/storage.py`：保留旧 JSONStorage API；业务状态走 KV adapter，向量数据走 pgvector。
...
- AI Core 内部调用边界。
- PostgreSQL auth/KV、pgvector、MinIO ObjectStore、Redis EventBus、Redis TaskQueue 和 Redis TaskLease。
...
- `identity` 的账户/会话库已使用 PostgreSQL；后续可按服务拆更细的 schema 和迁移版本管理。
