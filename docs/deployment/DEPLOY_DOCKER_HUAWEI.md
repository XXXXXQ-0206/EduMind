...
- 前端：`http://<你的ECS公网IP>/`
- 后端就绪检查：`http://<你的ECS公网IP>:5000/health/ready`
...
当前架构会单独启动 `bilibili-bridge` 容器；若异常，先看日志：

```bash
docker compose logs -f bilibili-bridge media-generation
```

已在镜像中预编译 `services/bilibili-mcp/dist/index.js`，并通过 `BILIBILI_MCP_ENTRY` 指向容器内 `/app/services/bilibili-mcp/dist/index.js`。
