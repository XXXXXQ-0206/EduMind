...
from utils.upload_objects import delete_upload_from_meta
...
async def _mutate_tasks(user: AuthUser, mutator: Callable[[List[Dict[str, Any]]], List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
    def apply(current):
        tasks = [dict(task) for task in current] if isinstance(current, list) else []
        return mutator(tasks)

    updated = await json_storage.update(_tasks_key(user.id), apply, default=[])
...
async def _broadcast(user_id: int, message: Dict[str, Any]) -> None:
    await publish_live_event(_planner_channel(user_id), message)
...
async def planner_ingest(payload: IngestRequest, user: AuthUser = Depends(require_auth)):
    ...
    await _mutate_tasks(user, lambda tasks: [task, *tasks])
...
async def planner_update(task_id: str, patch: Dict[str, Any], user: AuthUser = Depends(require_auth)):
    ...
    def apply_patch(tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        nonlocal before_status, task
        before_status = None
        task = None
        index = _task_index(tasks, task_id)
        if index < 0:
            return tasks
        next_task = dict(tasks[index])
        before_status = next_task.get("status")
        for key, value in patch.items():
            if key not in allowed:
                continue
            if key == "dueAt":
                next_task[key] = _coerce_due_at(value)
            elif key == "estMins":
                next_task[key] = _normalize_est_mins(value)
            elif key == "priority":
                next_task[key] = _normalize_priority(value)
            elif key == "status":
                next_task[key] = _normalize_status(value)
            elif key in {"tags", "steps"}:
                next_task[key] = ensure_string_list(value, limit=8, item_limit=80)
            else:
                next_task[key] = str(value or "").strip()
        next_task["updatedAt"] = _now_ms()
        tasks[index] = next_task
        task = next_task
        return tasks

    await _mutate_tasks(user, apply_patch)
    if not task:
        return JSONResponse(content={"ok": False, "error": "not found"}, status_code=404)
...
async def planner_delete(task_id: str, user: AuthUser = Depends(require_auth)):
    ...
    def remove_task(tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        nonlocal task
        task = None
        index = _task_index(tasks, task_id)
        if index < 0:
        ...
        return [item for item in tasks if str(item.get("id")) != task_id]

    await _mutate_tasks(user, remove_task)
    if not task:
        return JSONResponse(content={"ok": False, "error": "not found"}, status_code=404)
    for meta in task.get("files") or []:
        await delete_upload_from_meta(meta)
...
async def planner_plan(task_id: str, payload: PlanTaskRequest, user: AuthUser = Depends(require_auth)):
    ...
    if index < 0:
        return JSONResponse(content={"ok": False, "error": "not found"}, status_code=404)
    planned_task = await _plan_task_with_ai(dict(tasks[index]), cram=bool(payload.cram))
    task: Optional[Dict[str, Any]] = None

    def apply_plan(current_tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        nonlocal task
        task = None
        current_index = _task_index(current_tasks, task_id)
        if current_index < 0:
            return current_tasks
        merged = dict(current_tasks[current_index])
        for key in ("steps", "notes", "plan", "updatedAt"):
            if key in planned_task:
                merged[key] = planned_task[key]
        current_tasks[current_index] = merged
        task = merged
        return current_tasks

    await _mutate_tasks(user, apply_plan)
    if not task:
        return JSONResponse(content={"ok": False, "error": "not found"}, status_code=404)
...
async def planner_upload_files(task_id: str, file: List[UploadFile] = File(...), user: AuthUser = Depends(require_auth)):
    ...
    def add_files(tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        nonlocal task
        task = None
        index = _task_index(tasks, task_id)
        if index < 0:
            return tasks
        next_task = dict(tasks[index])
        next_task["files"] = [*(next_task.get("files") or []), *saved_files]
        next_task["updatedAt"] = _now_ms()
        tasks[index] = next_task
        task = next_task
        return tasks

    await _mutate_tasks(user, add_files)
    if not task:
...
async def planner_delete_file(task_id: str, file_id: str, user: AuthUser = Depends(require_auth)):
    ...
    def remove_file(tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        nonlocal target, task
        target = None
        task = None
        index = _task_index(tasks, task_id)
        if index < 0:
        ...
        for meta in next_task.get("files") or []:
        ...
        if not target:
            return tasks
        next_task["files"] = remaining
        next_task["updatedAt"] = _now_ms()
        tasks[index] = next_task
        task = next_task
        return tasks

    await _mutate_tasks(user, remove_file)
    if not target:
        return JSONResponse(content={"ok": False, "error": "not found"}, status_code=404)

    await delete_upload_from_meta(target)
...
async def planner_ws(websocket: WebSocket):
    ...
    await publish_live_event(channel, {"type": "ready", "sid": sid})
