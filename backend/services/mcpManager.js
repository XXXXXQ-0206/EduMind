...
class McpManager {
  ...
  resolveMcpEntry() {
    ...
    const fallback = path.resolve(__dirname, "../../services/bilibili-mcp/dist/index.js");
    const entry = candidateFromEnv ? path.resolve(candidateFromEnv) : fallback;

    if (!fs.existsSync(entry)) {
      throw new Error(
        `未找到 bilibili MCP dist 入口：${entry}。请先在 services/bilibili-mcp 目录执行 npm install && npm run build，或配置 BILIBILI_MCP_ENTRY。`
      );
    }
    return entry;
  }
  ...
  async init() {
    ...
    this.initializing = (async () => {
      ...
      this.client = new Client(
        {
          name: "edumind-bilibili-mcp-client",
          version: "1.0.0",
        },
        {
          capabilities: {},
